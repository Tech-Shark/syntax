use super::thread_local::GRAMMAR_MAP;
use crate::schema::grammar::{
    GrammarAnalysis, GrammarAnalysisMap, GrammarAnalysisResponse, GrammarCheckResult,
    GrammarUserInput,
};
...
pub fn gr_total_users() -> i64 {
    GRAMMAR_MAP.with(|map| map.borrow().len() as i64)
}

#[ic_cdk_macros::update]
pub async fn add_grammar_analysis(
    identity: String,
    ...
    GRAMMAR_MAP.with(|map| {
        ...
        grammar_analysis_map
            .map(|list| {
                list.analyses
                    .iter()
                    .map(|(key, grammar_analysis)| GrammarAnalysisResponse {
