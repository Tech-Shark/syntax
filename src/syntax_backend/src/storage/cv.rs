            ...
            if let Some(mut analyses) = res {
                ...
                if let Some(id) = new_idx {
                    let mut cva_list = CVAnalysisMap {
                        ...
                        last_analysed: get_current_time().to_string(),
                    };
                    cva_list.analyses.insert(
                        id.to_string(),
                        CVAnalysis {
                            idx: id.to_string(),
                            identity: identity.to_string(),
...
pub fn cv_total_users() -> i64 {
    CV_STORAGE_MAP.with(|map| map.borrow().len() as i64)
}
